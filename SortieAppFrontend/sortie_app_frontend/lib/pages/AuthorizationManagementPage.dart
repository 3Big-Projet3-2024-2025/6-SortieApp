...
class AuthorizationManagementPage extends StatelessWidget {
  ...
  const AuthorizationManagementPage({Key? key, required this.studentId}) : super(key: key);
...
class AutorisationListScreen extends StatefulWidget {
  ...
  // Ajoute le `studentId` au constructeur
  const AutorisationListScreen({Key? key, required this.studentId}) : super(key: key);
...
class _AutorisationListScreenState extends State<AutorisationListScreen> {
  ...
  Future<void> addOrUpdateAutorisation({
    ...
    try {
      ...
      final uri = Uri.parse('$apiUrl/Autorisations');
